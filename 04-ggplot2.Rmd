...
Make a histogram of the temp variable in the nh dataset
...
If we color by a continuous variable, we get a gradient, instead of individual colors:
...
A convenient way to label is with the labs() function, added as a component of the plot:
...
Plot flipper_length_mm vs. body_mass_g and color by sex.  Label your plot.
...
To change the axes min and max values, like we would with xlim or ylim in base R graphics, we have to use a scale function:
...
If you want to put multiple plots in a single image together, but they are facets of a single plot, see the patchwork package.
